...
def split_fill_and_stroke(path_node):
    ...
    if "fill" in style.keys() and style["fill"] == "none":
        if "stroke" not in style.keys() or style["stroke"] == "none":
          ...
          style_stroke = {"fill":"none", "stroke":"#000000"}
          ...
          path_node.set("style", simplestyle.formatStyle(style_stroke))
...
class MazakodronPrep(inkex.Effect):
    def effect(self):
        ...
        for node in self.document.xpath('//svg:path', namespaces=NSS):
            #inkex.errormsg(dir(node));
            #if node.get("d", "").lower().count("m") > 1:
            # There are multiple subpaths
            (fill,stroke) = split_fill_and_stroke(node)
            #    if fill is not None:
            #        fuse_subpaths(fill)
            #inkex.errormsg(type(node));
            if fill is not None:
              fill.getparent().remove(fill);
            if stroke is not None:
              stroke.set('color','#00000000');
